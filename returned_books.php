...
$issuedBooksRef = $database->getReference('issued_books')->getValue();
$returnedBooks = [];

if (!empty($issuedBooksRef)) {
    foreach ($issuedBooksRef as $issuedBookId => $issuedBookData) {
        if (!empty($issuedBookData['user_id']) && $issuedBookData['user_id'] === $userId) {
            $returnDate = $issuedBookData['return_date'] ?? null;
            if ($returnDate && $returnDate !== 'Not Returned') {
    ...
    <table class="table table-bordered table-striped">
        ...
        <tbody>
            <?php if (!empty($returnedBooks)) : ?>
                ...
                <?php endforeach; ?>
            <?php else : ?>
                <tr><td colspan="5" class="text-center text-success">No returned books found.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
